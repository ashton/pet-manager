...
let getDescription = (item: AppState.Breadcrumb.item) =>
  item->AppState.Breadcrumb.description->Option.getWithDefault(item.name)
...
let make = () => {
  ...
  <div className=contentHeader>
    ...
    <nav className=breadcrumb ariaLabel="breadcrumbs">
      <ul className=list> {state.breadcrumb->List.map(renderItem)->List.toArray->React.array} </ul>
    </nav>
  </div>
}
